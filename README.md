...
## TOPIC №1
**Если есть проблемы с доступом к подпискам GitHub через оригинальные ссылки** `https://raw.githubusercontent.com/igareck/` - **используйте альтернативно ссылки `GitHack RAW`** `https://raw.githack.com/`. 

**GitHack RAW** - caching proxy для raw-файлов из GitHub/GitLab/Bitbucket/Gitea/Codeberg.

**Альтернативные ссылки на каждую подписку расположены в разделе** `🪞 ЗЕРКАЛА 🪞`. **Менять в них ничего не нужно - просто замените ссылки у себя в клиенте и продолжайте пользоваться подпиской. В ближайшее время в дополнение к GitHack выложу список разнообразных альтернатив.**
...
**2. Githack RAW:** https://raw.githack.com

*Caching Proxy для raw-файлов из GitHub/GitLab/Bitbucket/Gitea/Codeberg.*

**Пункт 2 поможет только в случае Черных Списков. Актуально, если Роскомнадзор заблокирует GitHub.**
...
Githack RAW - информация актуальна всегда без задержек.
